...
def _build_model(config):
    ...
    # 默认配置使用方板；脚本也允许长和高不同，便于后续练习。
    sketch = model.ConstrainedSketch(
        name="biaxial_profile", sheetSize=max(length, height) * 2.0
    )
    ...
    del model.sketches["biaxial_profile"]

    # Abaqus 不预设单位；材料数值必须与配置声明的长度和应力单位一致。
    ...
    if (
        len(left_edges) == 0
        or len(right_edges) == 0
        or len(bottom_edges) == 0
        or len(top_edges) == 0
    ):
        raise RuntimeError("没有正确找到方板的四条边界，请检查几何参数。")

    # 左边限制 U1、下边限制 U2，既消除刚体运动又允许两个方向伸长。
    ...
    model.DisplacementBC(
        ...
        u1=UNSET,
        u2=float(analysis_config["top_edge_displacement"]),
    )
...
def _run_job(config, model_name):
    ...
    # 保存 CAE 文件，方便初学者在图形界面里检查两个方向的边界条件。
