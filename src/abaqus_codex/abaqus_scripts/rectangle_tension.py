# -*- coding: utf-8 -*-
"""在 Abaqus/CAE 2021 中建立、求解并读取二维矩形板拉伸模型。"""
...
def _build_model(config):
    ...
    # 先画二维矩形草图，再生成二维平面应力部件。
    sketch = model.ConstrainedSketch(
        name="rectangle_profile", sheetSize=max(length, height) * 2.0
    )
    ...
    del model.sketches["rectangle_profile"]

    # Abaqus 不预设单位；这里的材料数值必须和配置声明的单位制一致。
    ...
    # CPS4R 是四节点减缩积分平面应力单元，CPS3 用作必要的三角形退化单元。
    ...
    # 显式创建请求，避免依赖不同 Abaqus 版本是否自动生成默认请求。
    ...
    if len(left_edges) == 0 or len(right_edges) == 0 or len(anchor_vertices) == 0:
        raise RuntimeError("没有正确找到矩形板边界，请检查几何参数。")
...
def _read_results(config, instance_name, odb_path):
    ...
    try:
        ...
        for value in displacement_field.values:
            # caeModules 中存在同名 sum，这里显式循环以避免覆盖 Python 内置函数。
