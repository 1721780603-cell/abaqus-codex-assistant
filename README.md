...
左侧新增“接触怎么选”教学入口，按土木工程中的真实界面行为区分：无需接触、Tie、Embedded Region、接触对、General Contact、Cohesive contact 和 Connector/Coupling。指南会解释适用场景、必填参数、常见误用及结果检查项，但不会自动修改模型；接触创建仍需后续白名单执行器和用户确认。
