...
4. 使用窗口自动填入的命令，从几何、材料、截面、装配、分析步、相互作用检查、边界条件、网格、Job 一直做到结果报告。
5. 每一步先点击“生成计划”，核对对象、旧值、新值、风险和 mm-N-s-MPa 约定。
6. 除相互作用教学检查外，点击“应用修改”并在确认框选择“是”后才会执行。
7. Job 提交后等待 Abaqus 完成，再生成第 10 步计划；未完成的 Job 会被明确拒绝。
8. 回到 Abaqus 检查当前工作副本，并打开同目录的新建 `*_report_zh_001.md` 报告。
